...
void main() async {
  ...
  List<Template> sampleTemplates = await ts.getSampleTemplates(
    'assets/data/example_b.json',
  );
  ...
  List<Template> templates = await ts.getTemplateData(
    await ts.localFile(ts.templatesFName),
  );
  ...
  List<Template> sessions = await ts.getTemplateData(
    await ts.localFile(ts.sessionsFName),
  );
  ...
  runApp(
    MaterialApp(
      ...
      home: HomePage(initialTemplates: templates, initialSessions: sessions),
    ),
  );
}
